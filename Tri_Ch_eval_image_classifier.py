...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    with tf.variable_scope('InceptionV4', [images], reuse=None) as scope:
      ...
      if 'AuxLogits' in end_points:
        slim.losses.softmax_cross_entropy(
            end_points['AuxLogits'], one_hot_labels,
            label_smoothing=FLAGS.label_smoothing, weights=0.4,
            scope='aux_loss')
      slim.losses.softmax_cross_entropy(
          end_points['Logits'], one_hot_labels, label_smoothing=FLAGS.label_smoothing, weights=1.0)
    ...
    #  summaries.add(tf.summary.scalar('eval/losses/%s' % loss.op.name, loss))
    ...
    tf.logging.info("Wating for next evalution...")
    '''
