...
def main(_):
  ...
  with graph.as_default():
    ...
    # Select the network #
    ######################
    tf.logging.info("Base model name: %s" % FLAGS.model_name)
    ...
    with tf.device(deploy_config.inputs_device()):
      dataset_provider = slim.dataset_data_provider.DatasetDataProvider(
          dataset,
          num_readers=FLAGS.num_readers,
          common_queue_capacity=20 * FLAGS.batch_size,
          common_queue_min=10 * FLAGS.batch_size)
      [image, label] = dataset_provider.get(['image', 'label'])
